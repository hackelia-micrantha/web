...
`yarn test:cloudflare:bundle-budget` measures raw and gzip-compressed Worker bytes, verifies repository headroom, and writes `.cloudflare/functions/bundle-budget.json` for CI inspection.
