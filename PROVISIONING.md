# How to provision a new environment for micrantha
...
* Enter domains to secure (for example): `micrantha.com`
* Enter webroot path: `/home/deployer/apps/micrantha/shared/public/`

The `.pem` files created by `certbot-auto` need to be linked so Nginx can see them:

```
ln -s /etc/letsencrypt/live/micrantha.com/fullchain.pem /etc/ssl/micrantha.crt
ln -s /etc/letsencrypt/live/micrantha.com/privkey.pem /etc/ssl/micrantha.key
```
...
```
/opt/certbot/certbot-auto renew --webroot-path /home/deployer/apps/micrantha/shared/public/ --dry-run
```

Finally, add this to the root crontab to enable auto-renewal:

```
25 1,13 * * * /opt/certbot/certbot-auto renew --quiet --no-self-upgrade --webroot-path /home/deployer/apps/micrantha/shared/public/
```
